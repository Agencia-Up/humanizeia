...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const enabled = await isFeatureEnabled(admin, user.id, "jose_chat");
    if (body?.ping) return json({ enabled });            // sonda da UI (sem rodar o cérebro)
    if (!enabled) return json({ enabled: false, reason: "flag_off" });
