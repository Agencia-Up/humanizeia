...
function buildVehiclePhotoReply(memory: any, message: string) {
  ...
  ].filter(Boolean).filter((url, position, all) => all.indexOf(url) === position).slice(0, 5);
  ...
  return {
    ok: true,
    text: "Olha so como esta conservado 👌",
    source: "vehicle_photos_reply",
    vehicle,
    media: photos.map((file: string, photoIndex: number) => ({
      file,
      type: "image",
      caption: photoIndex === 0 ? `${vehicle?.label || vehicle?.modelo || "Veiculo"} - foto ${photoIndex + 1}` : `Foto ${photoIndex + 1}`,
    })),
  };
}
