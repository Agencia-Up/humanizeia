...
export function matchesConnectedAgentPhone(
  phone: string,
  rows: readonly { phone_number?: unknown; seller_member_id?: unknown }[],
): boolean {
  return rows.some((row) => !row?.seller_member_id && phonesMatch(String(row?.phone_number || ""), phone));
}
...
export async function identifyPedroContact(
  ...
  const [sellersResult, instancesResult, managersResult] = await Promise.all([
    supabase
      ...
      .limit(500),
    supabase.from("wa_instances").select("phone_number, seller_member_id").not("phone_number", "is", null).limit(5000),
  ...
  // A master connected line is another agent even if it was accidentally also
  // registered as a seller. A genuine seller instance has seller_member_id.
  if (matchesConnectedAgentPhone(phone, instancesResult?.data || [])) {
    return { kind: "internal", phone, remote_jid: input.remote_jid, reason: "phone_matches_connected_agent_instance" };
  }
  ...
  if (sellersResult?.error || instancesResult?.error || managersResult?.error) {
    return {
      kind: "unknown",
      phone,
      remote_jid: input.remote_jid,
      reason: `internal_lookup_error:${sellersResult?.error?.message || instancesResult?.error?.message || managersResult?.error?.message || "unknown"}`,
    };
  }
