  ...
  if (input.stock_result && !hasUsefulStock(input.stock_result)) {
    if (cameFromAd && !requested) {
      return {
        ok: true,
        text:
          `${name ? `${name}, ` : "Boa tarde! Sou o Carvalho, consultor aqui da Icom Motors. "}vi que voce veio por um anuncio da Icom. Pra eu conferir certinho no estoque, me diz qual modelo apareceu pra voce ou me manda um print do anuncio?`,
        source: "ad_context_needs_vehicle_confirmation",
      };
    }
