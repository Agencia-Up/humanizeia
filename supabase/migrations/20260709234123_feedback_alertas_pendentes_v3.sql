-- v3 do alerta "bom cliente em risco": traz mais contexto pro gerente AGIR na
-- hora — telefone do lead e horario do ultimo contato (ultima msg no wa_inbox).
-- vendedor_nome ja e o nome completo (tm.name). Gatilho/regua inalterados (bom
-- cliente + score<45 + sem venda + com vendedor). Mudou o retorno -> DROP+CREATE.
...
RETURNS TABLE(conversa_id uuid, lead_nome text, veiculo text, vendedor_nome text,
             motivo text, telefone text, ultimo_contato text)
