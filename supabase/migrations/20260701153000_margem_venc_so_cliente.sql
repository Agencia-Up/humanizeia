-- ============================================================================
-- Margem/Contas: próximo vencimento SÓ para cliente real (pagante)
-- ----------------------------------------------------------------------------
-- Contas internas da Logos (ADM/teste/vendedores) são "não pagante", mas toda
-- conta tem uma assinatura default (user_subscriptions.renewal_date) — então
-- elas apareciam com um "próximo vencimento" como se fossem clientes. Enganoso.
-- Aqui o proximo_venc só é preenchido quando a conta é pagante (tem venda paga
-- no site OU marcada manualmente). Não-pagante => null => a tela mostra "—".
-- RPC-only (o frontend já trata null): vale na hora, sem rebuild.
-- Idempotente (CREATE OR REPLACE), mantém SECURITY DEFINER + guard superadmin.
-- ============================================================================
    ...
    'contas', coalesce((
      SELECT jsonb_agg(row_to_json(t) ORDER BY t.pagante DESC, t.receita_brl DESC, t.custo_jose_brl DESC) FROM (
        SELECT a.user_id,
               ...
               -- próximo vencimento SÓ para cliente real (pagante); interna => null => "—"
               CASE WHEN (cp.recurrence_value IS NOT NULL OR coalesce(cr.ativo, false))
