...
begin
  ...
  v_url := coalesce(v_url, 'https://ezoltigtqgbmftmiwjxh.supabase.co/functions/v1/meta-lead-quality');
...
do $$
begin
  perform cron.unschedule('meta-lead-quality-daily-07');
exception when others then null;
end $$;

select cron.schedule(
  'meta-lead-quality-daily-07',
  '0 10 * * *',
  $$select public.cron_meta_lead_quality_daily_report()$$
);
