...
- RAIZ da contaminacao interesse-vs-troca (sintoma corrigido em v32, raiz pendente): `routePedroIntent` (intentRouter_20260525_sales.ts ~L58-68) extrai `extractLikelyModel(text)` e grava SEMPRE em `interesse.modelo_desejado`, mesmo quando o contexto e TROCA (branch trade_in ~L148 retorna o mesmo `extracted`). Resultado: lead diz "tenho um Spin pra trocar" -> interesse.modelo_desejado="Spin" (errado). Mitigado em v32 (resolver/helper/reply ancoram no veiculo APRESENTADO, nao no interesse), mas o campo continua sujo no banco e pode atrapalhar buscas memory-driven. FIX da raiz (com cuidado, NAO por regex simples — "quero trocar, tem Onix?" e interesse, nao troca): quando a mensagem for claramente posse/troca ("tenho um X","meu X","X na troca","dou o X") e SEM verbo de interesse ("tem/quero/procuro X"), rotear o modelo para `carro_troca` e NAO para interesse.modelo_desejado. Idealmente desambiguar via LLM no planner. Opcional: limpar states ja contaminados.
- COORDENACAO DE DEPLOY (raiz da "regressao" de 02/06): dev-aloan e Wander deployavam o `pedro-webhook-v2` de estados de Git DIFERENTES e se sobrescreviam (ex.: deploy do Wander as 18:20 derrubou o build v28/fotos do aloan; antes, deploy do aloan do `dev-wander` ia SEM os commits do Wander). DECISAO (02/06): **dev-aloan deploya SEMPRE da `main`** (uniao do codigo dos dois — Wander commita na main, aloan cherry-pick/commita na main). PENDENTE: (1) combinar com o Wander para ele tambem so deployar da `main` (nunca de branch local sem o que ja esta na main); (2) alinhar `dev-wander` = `main` (hoje dev-wander tem so commits do aloan; os do Wander estao so na main). SEMPRE conferir, apos deploy, `PEDRO_V2_BUILD` deployado via Management API (`/functions/pedro-webhook-v2/body`) — o 1o deploy as vezes serve bundle stale; redeploy resolve.
...
- ~~INVESTIGAR persistencia de memoria / resolucao de lead do Pedro v2~~ RESOLVIDO/RE-AVALIADO 2026-06-02: a afirmacao "veiculos_apresentados SEMPRE vazio" NAO se confirma mais. Re-checado em prod (25 states recentes): 15/25 tem `veiculos_apresentados>0` (Jeep Compass=3, Onix=3, Tracker, Mini Cooper, etc.) e ha `ultima_foto` salvo. A cadeia de save (`savePresentedVehicles` -> `savePhotoReference` -> `saveRecentConversationTurn`) preserva os campos (cada uma faz `{...current, campo}`) e `updatePedroMemoryFromIntent` usa `mergeObjects(current, patch)` (deep merge que NAO apaga `veiculos_apresentados`, ausente do patch). O `loadPedroMemory` carrega o `state` inteiro. Os states com `veic=0` sao leads em etapa inicial ou buscas que retornaram 0 (modelo fora do estoque: Polo/Duster/Gol/Golf) — comportamento correto, nao bug. O achado original (2026-06-01) provavelmente era de build anterior / conversas genericas. CONCLUSAO: a memoria persiste; o problema real de "so links, sem fotos" era o DETECTOR de pedido de foto estreito (corrigido no build `photo-detector-v28`, ver historico). Se reaparecer descasamento de lead por 9o digito BR, ai sim revisitar normalizacao de telefone/jid em `leadMemory.ts`.
