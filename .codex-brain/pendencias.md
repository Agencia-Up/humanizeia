...
- DEBOUNCE / CONCATENAR MENSAGENS (A FAZER — adiado, ponto 3 de 2026-05-30, o mais arriscado): o lead manda dados em varias mensagens seguidas (ex: nome, telefone, data de nascimento em 3 bolhas) e o Pedro v2 processa cada uma como um turno -> dispara varias respostas. Objetivo: agrupar mensagens proximas em UM turno. Abordagem proposta: debounce no `pedro-webhook-v2` — ao chegar a msg, esperar ~7s; se chegar outra, so a ULTIMA processa o bloco concatenado (checar via timestamp/wa_inbox se ha msg mais nova; se sim, sair sem responder). Concatenar as msgs do lead desde a ultima resposta do agente como o texto do turno. Arquitetural (toca webhook + entrada de texto do orquestrador). Fazer com MUITO cuidado para nao quebrar o fluxo que ja funciona; gated v2.
