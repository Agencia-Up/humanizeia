...
- COBRANCA POR CONVERSA (24h) e TEMPORARIA — REVISITAR depois de medir o custo real. Regra LIVE em prod desde 2026-06-03 (migration `20260603120000_pedro_conversa_24h_window.sql`, funcao `bill_pedro_lead`): lead novo=1 conversa, mesmo lead <24h da ultima atividade=mesma conversa (nao cobra), retorno 24h+=+1 conversa. ATENCAO (resolvido 2026-06-03): a funcao SOZINHA nao cobrava nada em prod porque o Pedro v33 NUNCA a chamava — quem efetivamente LIGA a cobranca e o TRIGGER `trg_bill_pedro_turn_log` (migration `20260603140000_bill_pedro_turn_log_trigger.sql`) em `pedro_v2_turn_logs`, que chama `bill_pedro_lead` a cada turno real de lead (ver historico 2026-06-03). Se algum dia o time do Pedro PARAR de gravar em `pedro_v2_turn_logs` (mudanca de schema/tabela no agente), a cobranca para de acontecer em silencio -> o trigger e EXCEPTION-hardened e depende dessa tabela. MONITORAR: confirmar periodicamente que `pedro_billed_leads`/`user_subscriptions.tokens_used` continuam crescendo com o trafego real. O Wander definiu "1 conversa = 1 atendimento cobrado POR ENQUANTO" so para medirmos via monitor o custo real por conversa (pra nos E pro cliente); DEPOIS ajustamos a regra/preco. PROXIMO PASSO quando o Wander pedir: olhar `pedro_billed_leads` (campo `charges` = nro de conversas/janelas de 24h por lead no ciclo; `raw_tokens` = custo interno acumulado) pra estimar custo medio por conversa e calibrar quota/preco. Cada cliente tem quota de conversas no `user_subscriptions.tokens_included` (basico 150, pro 300 — quota foi MANTIDA por plano, NAO rebaixada). NOTA: o reset de painel de 2026-06-03 zerou SO `brunolira0312@icloud.com` (uso estava 599077/150); as outras 20 contas ja estavam em 0.
- CLEANUP TRIVIAL: pasta de worktree `C:\Users\User\_wt_promo24h` ficou no disco (usada pra commitar a migration 24h na `main` sem merge cego; os comandos `git worktree remove`/`rm -rf` foram bloqueados pelo classificador de auto-mode). Remover manualmente (`git worktree remove C:\Users\User\_wt_promo24h --force` ou apagar a pasta + `git worktree prune`). NAO afeta o repo — o commit `c5761f2` ja esta em `origin/main`.
- RECONCILIACAO `staging` x `main` — FRONTEND DO CUSTO + MIGRATIONS PROMOVIDOS PRA `main` EM 2026-06-02 (commit `09e7377`, pushado pra origin/main -> EasyPanel rebuilda o frontend de prod). Como foi feito (PLANO SEGURO, sem repetir o incidente): NAO usei `git merge staging` cego; em cima da `main` limpa (v32) fiz `git checkout origin/staging -- <11 arquivos>` = 4 frontend (`src/pages/MeuPlano.tsx` card de custo, `src/pages/MargemInterna.tsx` painel interno de margem, `src/App.tsx` rota lazy, `src/components/layout/AppSidebar.tsx` link) + 7 migrations de custo (`20260602120000..180000`, ja aplicadas no banco de prod, trazidas so pra coerencia do repo). ZERO arquivo do Pedro tocado (`_shared/pedro-v2/**`, `pedro-webhook-v2`, `cron-lead-followup` intactos = v32 preservado). `npm run build` passou antes do commit. RESIDUAL: a linha do Pedro ainda diverge entre branches (staging=v27, main=v32) — quando for promover QUALQUER coisa de Pedro de staging->main, continuar resolvendo `_shared/pedro-v2/**` + `pedro-webhook-v2` + `cron-lead-followup` A FAVOR da `main` (v32). Deploy do Pedro continua SEMPRE da `main`. OBS sobre tokenMeter/telinha: confirmar se o v32 da `main` JA pluga o `tokenMeter` (billing fiado) no orquestrador ou se o arquivo so existe sem estar ligado — isso decide se a telinha de custo vai popular em prod (as RPC `cliente_meu_custo_overview`/margem ja estao no banco).
- [RESOLVIDO 2026-06-03] MERGEAR na `main` `create-evolution-instance`/`handle-instance-ban`/`cambio-update` — FEITO cirurgicamente (commit `9d909a4` em origin/main): via worktree em cima de `origin/main` (tip `303f2c2`, Pedro v33) dei `git checkout origin/staging -- <so esses 3 arquivos>`, conferido `git diff --cached --stat` = exatamente 3 arquivos (create-evolution-instance ownership check, handle-instance-ban service-role gate, cambio-update cron de cambio — arquivo novo) e ZERO arquivo do Pedro/brain. Push `303f2c2..9d909a4 -> main`. Confirmado depois: os 3 arquivos ficaram IDENTICOS entre main e staging, e `pedro-webhook-v2/index.ts` SEGUE diferente (v33 na main preservado, nao regrediu). NAO toquei `.gitignore` (main tem regras extra de ignore de seguranca do dev-aloan `scratch/`/`.tmp-*/`/`test_vision.cjs` que staging nao tem — pegar a versao de staging RE-EXPORIA). Push no main NAO redeploya edge function (deploy e via CLI), entao zero impacto no runtime de prod; so alinhou o repo pra futuros deploys saírem da versao segura. RESIDUAL: a `.gitignore` da staging ainda esta SEM essas 3 regras de ignore -> idealmente adicionar as mesmas linhas na staging tambem (pra ninguem commitar scratch com service-role key por la).
