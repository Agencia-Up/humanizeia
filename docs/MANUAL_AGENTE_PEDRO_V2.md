...
- 2026-07-30 - v225 - MODO HUMANO COM AGENTE DESLIGADO + PROJECAO SEM DUPLICATA: o inbound privado continua sendo persistido e projetado em Conversas IA/CRM quando o agente vinculado esta explicitamente inativo, mas sem resposta de IA, recuperacao automatica, follow-up T1/T2/T3 ou transferencia automatica. A RPC idempotente cria/reaproveita o lead pausado e revalida tenant/agente/instancia. A identidade canonica da conversa converge o registro orfao (instance_id null) com a instancia real sem mesclar duas instancias legitimas; conflitos de CRM ficam ambiguos em vez de serem apagados. Frontend preserva as operacoes humanas e os crons legados passaram a ignorar agente inativo/lead pausado. Regressao F2.95: 41/0.

- 2026-07-17 - v222 - ativacao multi-tenant preparada para a conta Avant Motors: agente Manu reativado com instance_id `0bb732d5-69f9-46cf-9269-9e732528c4ac`, RevendaMais validado (32 veiculos), transferencia e follow-up validados; escopo v3 ativo passa a ser resolvido antes do gate legado do v2 para que uma conta explicitamente dona do v3 nao seja bloqueada pela allowlist antiga.

- 2026-06-28 - v219 - GATE DE PILOTO PEDRO V3 ISOLADO: criado `pedroV3PilotGate.ts` e importado no webhook vivo para preparar a entrada do Pedro v3 sem risco de vazamento para clientes. RAIZ: antes de ligar o v3 no WhatsApp real, precisamos garantir por codigo que somente o tenant `ecb26258-ffe6-4fe2-9efc-8ab2fc3a61b0` com o agente `d4fd5c38-dd37-4da5-a971-5a7b7dfb9185` pode entrar no piloto; email/nome/instancia/primeiro ativo nao autorizam. FIX: gate deterministico por tenant+agent, modo `off|shadow|active` via `PEDRO_V3_PILOT_MODE`, default off e fallback seguro para v2 enquanto o handler ativo do v3 nao estiver conectado. +6 regressoes offline.
- 2026-06-28 - v220 - BRIDGE ATIVO ISOLADO PEDRO V3 (publicado, piloto ainda OFF): o webhook vivo encaminha somente o tenant/agente piloto exatos para o servico HTTP v3. IDs de evento/turno/conversa sao deterministas; retry de inbox pending e retomado; fallback v2 so ocorre com `ingested=false` explicito, evitando resposta dupla em timeout ou falha pos-insert. Midia inbound sem texto permanece no v2. Validado: 414/0 offline + bundle esbuild. IMPORTANTE: nao habilitar `PEDRO_V3_PILOT_MODE=active` antes da F2.6H ligar o callback Uazapi `messages_update` ao receipt delivered do outbox v3.
...
- 2026-06-25 - v204 - Corrige separadores corrompidos (???) no fallback de lista e bloqueia transferencia antes do funil quando ainda ha pergunta obrigatoria pendente; respostas curtas como "Gostei" agora seguem para a proxima pergunta do SDR antes do consultor.
- 2026-06-26 — v211 — Ledger de fotos por veículo + supressor de re-oferta (dor nº1 do dono: "manda fotos e depois pergunta se quer foto de novo"). `savePhotoReference` grava `fotos_por_veiculo[stableVehicleKey]` (chave marca|modelo|ano, ADITIVO, preserva `ultima_foto`). No pré-envio: `photoCtaDecision`+`stripPhotoReoffer` removem o CTA "quer ver as fotos?" quando as fotos do veículo em foco JÁ foram enviadas e o lead não pediu (só a frase de oferta sai, com fail-safe). `leadAsksForMorePhotos` corrigido: "manda mais OPÇÕES"/"tem mais opções?"/"mostra mais carros" não viram pedido de foto. Caso D (funil antes do handoff) já era enforced em runtime (`handoff_blocked_pending_funnel` via `nextFunnelQuestion`); extraído `funnelBlocksHandoff` puro como espelho testável (sem wiring redundante). +17 regressões offline (grupo `conversa` A–E/B-foto/strip). Validado: 354/0 offline + esbuild + dry-run v211 + convo-sim real. Deploy: pedro-webhook-v2 + pedro-recover-dropped.
- 2026-06-26 — v212 — NÃO PERDER O TRILHO (início da camada de continuidade): resposta de FINANCIAMENTO/ENTRADA não vira busca de estoque (caso real lead 98123-8305). O agente perguntou a entrada ("preciso saber o valor que você pode dar de entrada"); a lead respondeu "Não tenho" / "se precisa dar entrada não dá" / "vou pelo valor das parcelas que cabe no bolso" — e o agente DESPEJOU lista de carros (ouviu "valor" e virou stock_search). RAIZ DUPLA: (1) a pergunta de ENTRADA não era classificada como `perguntou_pagamento` (o regex exigia um verbo `paga/prefere/quer` que a frase de entrada não tem) → `pending_question` não era setado; (2) sem guard determinístico, o LLM reinterpretava "valor" como busca. FIX: (1) broaden de `classifyAgentReplyPending` + `classifyPendingQuestion` (entrada + `valor/quanto/preciso saber/pode dar` = pagamento); (2) detector puro `leadRespondsNoDownPaymentOrInstallmentConcern(msg, pending, lastAgent)` (sem-entrada / foco-parcela, com false-friends "não tenho interesse/troca/preferência"); (3) guard no `normalizePlan` (PRIORIDADE, antes de qualquer interpretação de orçamento): força `reply_only`/`financing`, nunca estoque/foto — EXCEÇÃO se o lead steera busca explícita (teto/carroceria → deixa buscar). Princípio geral: **havendo `pending_question`, a fala do lead é primeiro RESPOSTA a ela; só vira busca nova com intenção explícita de carro novo.** +11 regressões offline (grupo `conversa` Caso H). Validado: 365/0 offline + esbuild + dry-run v212 (busca normal intacta) + cadeia de persistência conferida (orchestrator:3200 grava pending_question). Deploy: pedro-webhook-v2 + pedro-recover-dropped. PRÓXIMO (mesma camada): resposta de troca/visita/dados não virar estoque; `pending_question` estruturado.
- 2026-06-26 - v216 - CAMADA CENTRAL DE ESTADO DA CONVERSA: cria `conversationState.ts` como fonte pura para pendencia do turno (`pending_question`), ultima fala do agente, relacao da resposta do lead e override de trilho antes do LLM/planner aceitar busca/foto/handoff. Ataca a raiz "agente sem nocao": resposta a dados/nome, detalhes de troca, confirmacao de visita e ping de presenca nao viram estoque/foto/transferencia por acidente; aceite curto de foto/opcoes vira acao correta. Planner recebe `decision_context.conversation_center` e o orquestrador persiste snapshot `conversation_center` junto do `pending_question`. +7 regressoes offline grupo `estado`. Build v216.
- 2026-06-26 — v215 — REPASSE/USADO/BARATO COMO BUSCA AMPLA MAIS EM CONTA: caso real Avant lead 92005-3580, áudio transcrito como "carro de repasse/carro usado" herdava S10 do anúncio/memória ou virava modelo-lixo `carro`+tipo `hatch`, zerava o estoque e o reply acabava citando Audi/Land Rover sem respeitar "barato/repasse". RAIZ: termos de segmento/preço (`repasse`, `usado`, `barato`) não eram invariante de busca ampla; `repasse` ainda entrava como token de modelo no matching. FIX: `wantsCheapBroadStock` no planner força `stock_broad`/`cheap_broad` só quando o modelo resolvido é genérico, limpa herança de anúncio/memória e preserva modelo real (ex.: Fox de repasse); `repasse/repasses/baixo/baixa` viram tokens fracos/ruído no stockSearch. +3 regressões offline (busca/planner).
- 2026-06-26 — v214 — BLOQUEIO DE HANDOFF NÃO ANUNCIADO: transferência anunciada só executa se a mensagem já enviada ao lead deixou claro que o consultor vai entrar em contato. Caso real lead 99793-2207: após responder "Qual parcela mensal ficaria confortável?", o orquestrador executou `trade_in_transfer_enforced` depois do envio e marcou handoff por trás; as mensagens seguintes do lead ficaram sem resposta. RAIZ: enforcements de transferência rodam na Etapa C depois do bloco de envio/salvamento, então mutações tardias em `reply.text` não chegam ao WhatsApp. FIX: snapshot da resposta realmente enviada + helper puro `shouldBlockUnannouncedHandoff`; se `brainReadyToTransfer/contextualIntent.needs_handoff` mas o texto enviado não passa em `transferMessageIsClear`, bloqueia handoff, restaura texto/source enviados e mantém o atendimento vivo. +3 regressões offline no grupo `transfer`. Validado: 376/0 offline + esbuild + dry-run v214 (build vivo e busca normal intacta). Deploy: pedro-webhook-v2 + pedro-recover-dropped.
- 2026-06-26 — v213 — NÃO PERDER O TRILHO (troca/avaliação): resposta do lead sobre VALOR da troca não vira estoque/foto. Caso real Francisco/Hilux: durante coleta da troca, lead disse "acho que o meu carro está em um valor maior" e o agente pulou para apresentação/foto de veículo. RAIZ: havia guard para oferta de troca, mas não para objeção de avaliação dentro de `pending_question=perguntou_troca`; redes de busca podiam reinterpretar o carro em memória como interesse novo. FIX: detector puro `leadRespondsTradeValueObjection(msg,pending,lastAgent)` com false-friends ("quero um carro maior") + guard prioritário em `normalizePlan` que força `reply_only`/`trade_in`, sem estoque/foto, exceto busca nova explícita (carroceria/teto). +8 regressões offline (grupos `troca` e `planner`). Validado: 373/0 offline + esbuild + dry-run v213 (build vivo e busca normal intacta). Deploy: pedro-webhook-v2 + pedro-recover-dropped.
- 2026-06-26 — v217 — CONVERSATION_CENTER (próxima fatia, memória real de atendimento): `last_stock_offer` + `current_vehicle_focus`. Ao enviar uma lista, `savePresentedVehicles` passa a persistir `last_stock_offer` (categoria `tipo_veiculo` + teto `preco_max` + `vehicle_keys` estáveis marca|modelo|ano + count + timestamp). O override `options_acceptance` (aceite de "ver opções") agora HERDA a mesma categoria/faixa da última oferta → "mais opções" não mistura SUV/sedan/hatch nem repete (o dedup `opcoes_listadas_keys` complementa); antes buscava só `stock_broad` SEM categoria (raiz do "mais opções fora da categoria"). `current_vehicle_focus` exposto no `buildConversationCenter` como VIEW do `veiculo_em_foco` já persistido (alimenta o LLM + prepara o invariante de "esse/dele/tem laudo?"). Funções puras novas em `conversationState.ts`: `buildLastStockOffer`, `stockOfferVehicleKey`. Reaproveita estado existente (NÃO duplica veiculo_em_foco/veiculos_apresentados). +6 regressões offline grupo `estado` (392/0). Validado: 392/0 + esbuild + dry-run v217 (busca normal intacta). Deploy: pedro-webhook-v2 + pedro-recover-dropped. PRÓXIMO (mesma camada): invariante de `current_vehicle_focus` (referência "esse/dele" não vira busca nova), `photo_ledger` formalizado no center, `funnel_state` em slots, `next_action_reason`.
- 2026-06-26 - v218 - FOCO CONTEXTUAL EXECUTAVEL + LISTA UNICA: caso real no agente de teste, apos listar QQ/Sandero/Peugeot 208 o lead escreveu "gostei do peujot" + "tem foto?"; o agente ignorou a selecao, repetiu a lista e mudou a formatacao. RAIZ: v217 apenas expunha `current_vehicle_focus`, mas nao resolvia a fala contra `veiculos_apresentados`; o seletor de foto fazia match literal e o reply de estoque tinha renderizacao variavel por LLM. FIX: `resolvePresentedVehicleReference` usa o working set realmente mostrado como dicionario contextual (ordinal, modelo numerico e fuzzy com margem/ambiguidade segura), fixa `veiculo_em_foco` antes do planner e forca `photo_request` quando ha pedido explicito; modelo novo explicito e token desconhecido vencem a memoria. Listas com 2+ resultados passam por um unico renderer deterministico numerado. +7 regressoes de estado, incluindo typo->Peugeot->midia, sem referencia, empate e Renault Duster nao sequestrado por Sandero. Validado offline + bundle; deploy webhook + recover.
