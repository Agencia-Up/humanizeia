...
import { FolderOpen, Search, Grid, List, Image, LinkIcon, Sparkles, Trophy, TrendingUp, DollarSign, Copy, Loader2, Zap } from 'lucide-react';
...
export default function CreativeLibrary() {
  ...
  const fetchAds = useCallback(async () => {
    // Step 1: Fetch ads with basic creative info + insights
    ...
    // Step 2: Collect unique creative IDs
    ...
    // Step 3: Fetch high-res thumbnails for each creative (batch via IDs endpoint)
    ...
    if (creativeIds.length > 0) {
      try {
        ...
        // crResult is { [creative_id]: { thumbnail_url, id } }
        if (crResult && typeof crResult === 'object') {
          for (const [id, data] of Object.entries(crResult as Record<string, any>)) {
            if (data?.thumbnail_url) {
              hiResMap[id] = data.thumbnail_url;
            }
          }
        }
      } catch {
        // Fallback: use low-res thumbnails if hi-res fetch fails
      }
    }

    // Step 4: Merge hi-res thumbnails into ads data
    if (adsResult?.data) {
  ...
  // Get best available image URL for an ad
  ...
  const { sortedAds, activeCount, topCtrAd, totalSpend } = useMemo(() => {
    ...
    // Calculate KPIs from all ads (before status filter for global KPIs)
...
Responda em Markdown formatado com headers (##), **negrito** para títulos e CTAs, e separadores (---) entre variações. NÃO retorne JSON.`;
  ...
  const hasCachedData = allAds.length > 0;

  if (!isConnected && !isLoadingConn && !hasCachedData) {
    return (
      <MainLayout>
        <div className="flex flex-col items-center justify-center gap-4 py-20">
          <LinkIcon className="h-12 w-12 text-muted-foreground" />
          <h2 className="text-xl font-semibold">Conecte seu Meta Ads</h2>
          <p className="text-muted-foreground text-center max-w-md">Para ver seus criativos reais, conecte sua conta.</p>
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold lg:text-3xl">Creative Library</h1>
            <p className="text-muted-foreground">Seus anúncios reais do Meta Ads — ordenados por performance</p>
          </div>
        </div>

        {/* Reconnection banner */}
        {!isConnected && !isLoadingConn && hasCachedData && (
          <Card className="border-yellow-500/30 bg-yellow-500/10">
            <CardContent className="flex items-center justify-between gap-4 p-4">
              ...
              <Button size="sm" variant="outline" onClick={() => navigate('/settings')} className="border-yellow-500/30 text-yellow-500 hover:bg-yellow-500/10">
                Reconectar
              </Button>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div>
                <p className="text-2xl font-bold">{topCtrAd ? `${Number(topCtrAd.insights?.data?.[0]?.ctr || 0).toFixed(2)}%` : '—'}</p>
                <p className="text-xs text-muted-foreground truncate max-w-[120px]">Melhor CTR</p>
              </div>
            </CardContent>
          </Card>
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
                  ...
                  <Card className="group relative border-border/50 bg-card/50 backdrop-blur-sm overflow-hidden transition-all hover:border-primary/30 hover:shadow-lg">
                      ...
                      <div className="relative aspect-[4/5] cursor-pointer" onClick={() => setPreviewAd(ad)}>
                        <img
                          ...
                          onError={(e) => {
                            ...
                            if (originalThumb && target.src !== originalThumb) {
                              target.src = originalThumb;
                            }
                          }}
                        />
                      </div>
                    ...
                    <CardContent className="p-3 space-y-2">
                      ...
                      <Button
                        ...
                        onClick={() => handleGenerateVariations(ad)}
                      >
                ...
                {variationResult && (
                  <div className="space-y-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="gap-1.5"
                      onClick={() => copyToClipboard(variationResult)}
                    >
