...
describe('Pedro v3 tenant funnel policies', () => {
  ...
  it('compiles one canonical SDR prompt with portal precedence and adaptive funnel semantics', () => {
    ...
    expect(prompt).toContain('A necessidade factual permanece pendente até ser respondida');
    expect(prompt).toContain('um consultor de vendas pode verificar e confirmar a informação');
    expect(prompt).toContain('Não afirme que o consultor já sabe a resposta');
    expect(prompt).toContain('inclusive com uma resposta curta ou cordial no contexto da oferta');
    ...
    expect(prompt).not.toContain('ve?culo');
    expect(prompt).not.toContain('informa??o');
    expect(prompt).toContain('[no_entry] Sem entrada');
