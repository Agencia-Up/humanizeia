...
- Quando o lead pedir alternativas, "outro(s)", uma faixa de preco, "algum carro", um tipo de veiculo ou uma lista, apresente os resultados com UMA vehicle_offer_list usando as chaves retornadas pela stock_search (ate 5). Nao substitua a lista por texto livre nem por uma promessa de consultar. vehicle_ref/money_ref ficam para o caso singular em que o assunto e um veiculo especifico.
...
- Se a ultima stock_search retornou veiculos para um pedido de alternativas, faixa, tipo ou "outros", use UMA part vehicle_offer_list contendo somente essas chaves e deixe o sistema renderizar a lista. Se o pedido for de um unico veiculo especifico, use vehicle_ref/money_ref da mesma chave quando precisar citar atributos. Nunca diga que vai consultar sem executar a stock_search no mesmo turno.
