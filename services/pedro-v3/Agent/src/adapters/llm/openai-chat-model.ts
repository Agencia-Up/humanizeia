...
function operationInstructions(operation: OpenAiChatOperation): string {
  ...
  if (operation === "propose") {
    return [
      ...
      "",
      "CONTEXT in request.turn.context — USE IT; this is a stateful conversation, NOT isolated messages:",
      "  - recentTranscript: recent lead/agent turns. lastAgentMessage: your own last message (give continuity, do not repeat it).",
      "  - alreadyIntroduced: if TRUE you ALREADY greeted and introduced yourself — NEVER greet or present yourself (name/role/company) again; just continue.",
      "  - conversationFacts: what is already known (name, city, interest, budget...). NEVER ask again for something already here.",
      "  - currentObjective: the question/offer awaiting the lead's answer. If the lead just answered it, treat the message as that answer and move forward.",
      "  - lastCommercialInterest + facts: if the lead named a specific vehicle, the engine ALREADY ran stock_search for you and the result is in facts — decide from facts; do NOT re-query the same model.",
      "",
      ...
      '  - VEHICLES ARE GROUNDED: only present or confirm a vehicle that appears in facts (stock_search/vehicle_details). If the lead asked for a specific model and it IS in facts, present it. If it is NOT in facts (not in stock), do NOT claim to have it — plan to say it was not found right now and to offer real alternatives that ARE in facts.',
  ...
  return [
    ...
    '  {"type":"vehicle_ref","vehicleKey": <string>, "field":"marca"|"modelo"|"ano"|"km"|"cambio"|"cor"}  // ATTRIBUTE value comes from the EXACT VehicleFact',
    ...
    '  {"type":"vehicle_offer_list","vehicleKeys":[<string>, ...]}  // PREFERRED way to present vehicles from stock',
    "Use request.turn.context for continuity: do NOT repeat lastAgentMessage; if alreadyIntroduced is TRUE do NOT restate your name/role/company (you already did); rely on conversationFacts instead of re-asking what is known.",
    ...
    "ATTRIBUTE GROUNDING (hard rule): year, km, transmission (cambio) and color (cor) come ONLY from the vehicle's VehicleFact — emit a vehicle_ref part with the right field for the EXACT vehicleKey. NEVER state or contradict cambio/cor/km/ano in free text. If the attribute is not in the facts, say you will confirm it — do NOT invent a value.",
    "NO DUPLICATION around refs (fixes 'automatico Automatico' / '130.000 km km'): a vehicle_ref renders the COMPLETE value already — km renders as '130.000 km' (WITH the unit), cambio as 'Automático', cor as 'Branco', ano as '2016'. So do NOT write the unit ('km') next to a km ref, and do NOT repeat the same word in free text right before/after the ref. Write connective text only (e.g., text 'Esse é ' + vehicle_ref cambio + text ', bem econômico.').",
