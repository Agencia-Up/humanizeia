...
import type { ClaimExtractor, ProposedDecision, QueryInputMap, TurnInterpretation } from "../domain/decision.ts";
...
export type ExplicitSearchResult =
  | { readonly kind: "offer"; readonly label: string; readonly vehicles: VehicleFact[]; readonly missingLabels: readonly string[] }
...
export async function resolveExplicitSearchIntent(args: {
  ...
  return vehicles.length > 0 ? { kind: "offer", label, vehicles, missingLabels } : { kind: "none", label };
}
...
export function buildExplicitSearchTurnOutput(result: ExplicitSearchResult, turnId: Id): TurnOutput {
  if (result.kind === "none") {
    const text = `No momento nao tenho ${result.label} no nosso estoque. Posso te mostrar opcoes parecidas - quer que eu filtre por preco, tipo (SUV, hatch, sedan) ou outra marca?`;
    return textTurn(turnId, text, "explicit_not_found", `Sem ${result.label} no estoque - honesto, sem inventar nem cair em modelo antigo.`);
  }
  const missing = result.missingLabels.length > 0
    ? `\n\nNao encontrei agora: ${result.missingLabels.join(", ")}.`
    : "";
  const text = `Encontrei estas opcoes pra voce:\n\n${renderVehicleOfferList(result.vehicles, { maxItems: 5 })}${missing}\n\nQuer ver as fotos de algum desses ou prefere agendar uma visita?`;
  ...
  return { ...textTurn(turnId, text, "explicit_offer", `Oferta da busca explicita do turno (${result.label}), ancorada no estoque.`), renderedOfferContext: items };
}

function textTurn(turnId: Id, text: string, reasonCode: string, reasonSummary: string): TurnOutput {
  const proposal: ProposedDecision = {
    proposedAction: "reply", facts: [],
