export type BrazilChannelTime = {
  ...
  readonly greeting: "Bom dia" | "Boa tarde" | "Boa noite" | null;
};
...
export function getBrazilChannelTime(now: string): BrazilChannelTime {
  ...
  if (!Number.isFinite(date.getTime())) {
    return { timeZone: "America/Sao_Paulo", localDateTime: null, period: null, greeting: null };
  }
  ...
  const period = Number.isFinite(hour)
    ...
    : null;
  const greeting = period === "manha" ? "Bom dia" : period === "tarde" ? "Boa tarde" : period === "noite" ? "Boa noite" : null;
  return { timeZone, localDateTime, period, greeting };
}

export function invalidBrazilGreeting(text: string, now: string): string | null {
  const expectedGreeting = getBrazilChannelTime(now).greeting;
  if (!expectedGreeting) return null;
  const greeting = text.match(/\b(?:bom|boa)\s+(?:dia|tarde|noite)\b/iu)?.[0]?.toLocaleLowerCase("pt-BR");
  if (!greeting) return null;
  const expected = expectedGreeting.toLocaleLowerCase("pt-BR");
