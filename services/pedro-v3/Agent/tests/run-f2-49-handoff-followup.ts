...
const degradedAnchor = record({
  ...
  idempotencyKey: "turn-degraded:message",
  onSuccess: [{
    op: "append_assistant_turn",
    effectId: "turn-degraded:message",
    turn: { role: "agent", text: "Tive uma instabilidade para concluir esta resposta.", at: ANCHOR, authoring: "technical_fallback" },
  }],
});
check("[P4a] fallback tecnico aceito nunca arma T1/T2/T3", evaluateFollowup({
  state: state(), outbox: [degradedAnchor], rules: rules.followup, now: NOW,
}).reason === "anchor_degraded");
