...
const photoBrainFor = (vehicleKey: string): BrainResponder => (frame, obs) => {
  ...
  return photo?.data.photoIds.length
    ...
    : qU({ tool: "vehicle_photos_resolve", input: { vehicleRef: { kind: "vehicle", key: vehicleKey } } }, understanding);
};
...
async function main(): Promise<void> {
  ...
  // ── E) ANÚNCIO ESPECÍFICO com snapshot vazio: fala do Compass 2019; foto "dele" resolve o Compass ──
  {
    ...
    const t2 = await c.t("me manda fotos dele", photoBrainFor(COMPASS.vehicleKey));
  ...
  // ── I) MAIS FOTOS com snapshot vazio: próximo lote do MESMO veículo, 0 stock_search ──
  {
    ...
    const t2 = await c.t("me manda fotos do onix", photoBrainFor(ONIX.vehicleKey));
    ...
    const t3 = await c.t("tem mais fotos?", photoBrainFor(ONIX.vehicleKey));
