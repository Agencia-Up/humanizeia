...
async function main(): Promise<void> {
  ...
  const report = [
    ...
    "## Conversa simulada",
    "",
    "| T | Lead | Resposta Pedro v3 | Intent | Tools | Effects | Slots | Fonte |",
    "|---:|---|---|---|---|---|---|---|",
    ...turns.map((turn) => `| ${turn.turnIndex} | ${sanitize(turn.leadBlock).replace(/\|/g, "/")} | ${sanitize(turn.response).replace(/\|/g, "/")} | ${turn.primaryIntent ?? "-"} | ${turn.toolsRequested.join(", ") || "-"} | ${turn.effects.map((effect) => effect.kind).join(", ") || "-"} | ${sanitize(slotText(turn)).replace(/\|/g, "/")} | ${turn.responseSource ?? "-"} |`),
    "",
    "## Briefing final",
    "",
